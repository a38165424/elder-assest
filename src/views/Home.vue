<template>
  ...
  const validateGender = (blur) => {
    if (formData.value.gender.length === 0) {
      if (blur) errors.value.gender = "Please select at least one gender option";
    } else {
  ...
  const validateResident = (blur) => {
    if (formData.value.isAustralian === false) {
      if (blur) errors.value.resident = "Please confirm whether you are an Australian resident";
    } else {
